...
Aditionally, we built six different supervised learning models using the **caret** framework to compare model performance with other popular methods commonly used to classify stroke risk.
...
**Interpretation of the Correlation Heatmap of Key Numeric Predictors**

All correlations are weak to moderate (0.00–0.26) indicating no multicollinearity concerns. 

* **Age** shows small but meaningful positive correlations with: Average Glucose (0.24), hypertension (0.26) , and heart disease (0.26). These findings are consistent with known aging-related cardiovascular risk patterns. 

* **BMI** has very weak correlations with all other predictors (0.04 – 0.16) suggesting it behaves independently in this dataset.

* **Average glucose** moderately correlates with hypertension (0.17) and heart disease (0.14). These findings aligns with metabolic/vascular relationships.

* **Hypertension** and **heart disease** are weakly correlated (0.11) showing they are related but not redundant.
...
- **Hypertension** has a significant positive effect on stroke risk (p < 0.001), indicating hypertensive individuals are more likely to experience stroke.

- **Average glucose level** is also a important predictor (p = 0.0106).
Higher glucose values modestly increase stroke risk.

- **Heart disease** shows a positive association but is not significant (p = 0.18920).
This suggests a potential effect, but not statistically significant in this model.

- **Smoking** has likewise positive association but is not significant (p = 0.15814), indicating an increased risk among smokers, but the evidence is not too much strong.

- **BMI**, **gender**, and **marital status** show no meaningful statistical association with stroke in this dataset (all p > 0.05). These variables did not contribute substantially to prediction after accounting for other factors.
...
- **Age** (OR = 1.081; CI: 1.063 – 1.099).
Age is the strongest continuous predictor. Each additional year of age increases the odds of stroke by about 8%, and the confidence interval does not include 1, indicating strong statistical significance.

- **Hypertension** (OR = 2.513; CI: 1.644 – 3.805).
Individuals with hypertension have roughly 251% higher odds of stroke compared to those without hypertension, the confidence interval does not include 1, indicating strong statistical significance.

- **Heart disease** (OR = 1.438; CI: 0.821 – 2.436).
Heart disease increases stroke odds by about 43%, but the CI includes 1, implying the association is positive but not statistically strong in this dataset.

- **Average glucose level** (OR = 1.005; CI: 1.002 – 1.009).
Higher glucose levels are associated with slightly increased stroke risk. Though the effect is small, the CI indicates significance, aligning with known metabolic risk patterns.

- **BMI** (OR = 1.003; CI: 0.972 – 1.033).
...
- **Smoking Status** (OR = 1.199; CI: 0.930 – 1.542).
Smoking increases the odds by about 20%, but the confidence interval includes 1 implying the association is positive but not statistically strong in this dataset.

- **Gender** (OR = 0.901; CI: 0.602 – 1.338).
Gender is not statistically significant.

- **Ever married** (OR = 0.588; CI: 0.338 – 1.070).
...
The model identified 0 out of 54 actual stroke cases. This shows the model fails to detect stroke cases, which is common in rare-event medical datasets.

- Specificity (True Negative Rate) = 0.99.
The model correctly classified almost all non-stroke cases. It is extremely good at predicting “No stroke,” which dominates the dataset.

- Positive Predictive Value (Precision) = 0.
The model has not predict a single correct value, further analysis needed.

- Negative Predictive Value = 0.946
Most “No” predictions are correct, matching the overall class imbalance.

- Kappa = -0.0057
Kappa measures agreement beyond chance. A negative Kappa value indicates that the model's performance is worse than random chance.

- Balanced Accuracy = 0.498
When weighting sensitivity and specificity equally, the model performs at chance level (~50%). Confirms that stroke detection is weak.
...
The logistic regression model achieves high accuracy only because the negative class dominates. It detects no true stroke cases, giving extremely poor sensitivity.
...
The AUC = 0.779, which indicates good discriminative performance.
...
Tree-based models (Decision Tree, GBM) achieve slightly higher sensitivity than LR, but only marginally (still around 1–2%). KNN and SVM detect 0 stroke cases at this threshold, despite high accuracy.
...
Random Forest (AUC = 0.723) and GBM (AUC = 0.756) also show good discriminative ability and are close competitors to logistic regression.

KNN (AUC = 0.667) performs moderately, better than random guessing but weaker than the tree-based and regression models.

Decision Tree (AUC = 0.648) and SVM (AUC = 0.632) show the lowest AUC values, indicating weaker predictive performance.
...
- With a lower decision criterion of 0.2, the model successfully identifies 12 out of 59 stroke cases (sensitivity = 22%), compared to no case with the default 0.5 threshold.

- Specificity remains high at 93%, indicating that the majority of non-stroke patients are still properly categorized as "no stroke" (889 out of 953).

- While overall accuracy declines from 94% to 89%, balanced accuracy improves (from ≈0.50 to ≈0.58), indicating a greater balance of sensitivity and specificity.
